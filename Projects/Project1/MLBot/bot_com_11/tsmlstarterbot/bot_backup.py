...
def decision_bot(game):
    ...
    while True:
        ...
        for ship in game_map.get_me().all_ships():
            ...
            #closest_valid_entities = closest_viable_planets + closest_enemy_ships
            #closest_valid_entities = (merge(closest_enemy_ships, closest_viable_planets))#need to figure this out. combining these lists and keeping them sorted
            #closest_valid_entities = [entities_by_distance[distance][0] for distance in entities_by_distance if isinstance(entities_by_distance[distance][0], hlt.entity.Ship) and entities_by_distance[distance][0] not in team_ships] + [entities_by_distance[distance][0] for distance in entities_by_distance if isinstance(entities_by_distance[distance][0], hlt.entity.Planet) and (entities_by_distance[distance][0].is_owned() == False or entities_by_distance[distance][0].owner == game_map.get_me().id)]
            ...
            """if ship.can_dock(closest_viable_planets[0]):

                command_queue.append(ship.dock(closest_viable_planets[0]))

            #working on this above

            elif ship.calculate_distance_between(closest_viable_planets[0]) < 50:

                navigate_command = minePlanet(closest_viable_planets[0])
            ...
            if firstTurn: #len(team_ships) <= 3: #and ship.id != modelshipid:
            ...
            """
                ...
                if len(closest_enemy_ships) <= 5:
                    if ship.id == modelship.id:
                        if len(closest_viable_planets) > 0: #To fix, when no empty planets left. I think fixed.
                        ...
                        if ship.calculate_distance_between(closest_enemy_ships[0]) < 120:
                            for compareShip in team_ships:
                                if ship.id == compareShip.id:
                                    continue
                                elif ship.calculate_distance_between(compareShip) <= 5:
